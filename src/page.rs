    ...
    unsafe {
        ...
        for i in 0..num_pages - pages {
            ...
            if found {
                ...
                // structure per 4096 bytes starting at ALLOC_START
            ...
            if (*beg).is_taken() {
                ...
                loop {
                    ...
                    if (*beg).is_last() {
                        ...
                        crate::print!("0x{:x}: {:>3} page(s)", memaddr, (end - start + 1));
                        crate::println!(".");
                        break;
                    }
                    beg = beg.add(1);
...
// Helper functions to convert the enumeration into an i64, which is what our page table
// entries will be
impl EntryBits {
    pub fn val(self) -> i64 {
        self as i64
...
pub fn map(root: &mut Table, vaddr: usize, paddr: usize, bits: i64, level: usize) {
    ...
    // 这里找到，虚拟地址中的vpn[2]对应的目录项
    let mut v = &mut root.entries[vpn[2]];
    // Now, we're going to traverse the page table and set the bits
    // properly. We expect the root to be valid, however we're required to create anything beyond the root.
    ...
    // So, (0..2) will iterate 0 and 1.(0..2).rev() => 1, 0
    for i in (level..2).rev() {
        // 如果lv1 未建立，第一次的目录项invalid，则分配page
        // println!("vaddr = {:x}, i = {}, v.is_valid = {}", vaddr, i, v.is_valid());
        if !v.is_valid() {
            // Allocate a page
            let page = zalloc(1);

            // The page is already aligned by 4096, so store it directly
            // Then page is stored in the entry shifted right by 2 places
            // entry 存在 页地址 右移动两字节地方，这样就可以通过页地址找到entry了
            // 每个entry，对应一个页或者对应下一个Table(512 x 8 = 4096也是一个页长度)
            //  entry|page
            //     2 |page
            v.set_entry((page as i64 >> 2) | EntryBits::Valid.val());
            // println!("page = {:x}", page as usize);
            // println!("v.is_valid = {}", v.is_valid())
        }

        // 找到lv1设置的entry
        let entry = ((v.get_entry() & !0x3FF) << 2) as *mut Entry;
        // 将lv2设置的entry,移动vpn[2]字节，之后作为新的Entry,这个是基址偏移?
        // 这里说明这个entry只是新新建立的数组及地址而已,entry: *mut Entry
    ...
    let entry = (ppn[2] << 28) as i64 | // PPN[2] = [53:28]
                     ...
                     EntryBits::Valid.val(); // Valid bit
                                             // 将虚拟地址找到的entry，设置成物理地址的entry，就完成映射？？
                                             // 这个最终找到的entry项是通过虚拟地址的vpn[0..2]找到的,那么将这个找到的项，内容设置成物理地址的entry，也就是虚拟地址索引找到entry就变成指定的地址了？？
                                             // 再思考：
                                             // 1.虚拟地址vpn[0..2]只是用来做Table的索引偏移的，如vpn[2]用来root.entries[vpn[2]],然后建立vpn[1]的table,再将新建的Table上获取到entries,这个entries + vpn[1],
                                             //   再建立vpn[0]对应的Table，再将新建Table获取entries，将这个entries + 将vpn[0] 得到最终的entry v
                                             // 2.we should be at VPN[0] and v should be pointing to our entry.
                                             // 3. 这个最后的指向entry的指针v的内容应该是物理地址的entry内容
                                             // 4.最终完成了虚拟地址=> 物理地址的映射
    v.set_entry(entry);
}
...
/// NOTE: This does NOT free root directly. This must be freed manually.
...
pub fn unmap(root: &mut Table) {
    ...
    for lv2 in 0..Table::len() {
        ...
        if entry_lv2.is_valid() && entry_lv2.is_branch() {
            ...
            let memeaddr_lv1 = (entry_lv2.get_entry() & !0x3ff) << 2;
            let table_lv1 = unsafe {
                // Make table_lv1 a mutable reference instead of a pointer
                (memeaddr_lv1 as *mut Table).as_mut().unwrap()
            };
            for lv1 in 0..Table::len() {
                let ref entry_lv1 = table_lv1.entries[lv1];
                if entry_lv1.is_valid() && entry_lv1.is_branch() {
                    let memaddr_lv0 = (entry_lv1.get_entry() & !0x3ff) << 2;
                    // The next level is level 0, which cannot branches
                    // therefore, we free here
                    dealloc(memaddr_lv0 as *mut u8);
                }
            }
            dealloc(memeaddr_lv1 as *mut u8);
        }
    }
}
