...
use core::{mem::size_of, ptr::null_mut, alloc::{GlobalAlloc, Layout}};
...
pub fn kzmalloc(_size: usize) -> *mut u8 {
    let size = crate::page::align_val(_size, 3);
...
// We want to use these sparingly since we have a coarse-grained allocator.
...
unsafe impl GlobalAlloc for OsGlobalAlloc {
    unsafe fn alloc(&self, layout: Layout) -> *mut u8 {
        ...
        // we divide by PAGE_SIZE, we get exactly the number of pages necessary.
...
/// reveals that we're creating a new structure and not just copying a value.
...
pub fn alloc_error(l: Layout) -> !{
    panic!("Allocator failed to allocate {} bytes with {}-byte alignment.",
        l.size(), l.align());
}